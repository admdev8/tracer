...
\IFRU{Если указана опция \IT{-fpu\_never}, состояние регистров FPU никогда не будет выводиться.}{If \TT{-fpu\_never} option is set, FPU registers state will not be dumped.}

\IFRU{Если указана опция \IT{-dump\_xmm}, состояние всех регистров XMM также будет выведено, если только регистр не пуст.}{If \TT{-dump\_xmm} option is set, each XMM registers state will be dumped too, unless it is empty.}
...
\IFRU{Замечание: момент когда tracer меняет состояние регистров это момент \IT{перед} тем как текущая инструкция будет исполнена. Изменение флагов \IT{перед} инструкциями TEST или CMP бессмысленно.}{Note: the moment when gt can change registers state is the moment \IT{before} current instruction is executed. Changing flags \IT{before} TEST or CMP instructions is useless.}
