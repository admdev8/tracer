...
\IFRU{Названия PE-секций так же делаются специальными символами}{PE-section names are also used
as special symbol names}.
\IFRU{Например, символ}{For example,} \TT{file.exe!.data} \IFRU{имеет адрес начала секции}{symbol has
address of the} \TT{.data} \IFRU{в}{section begin in} \TT{file.exe}.
